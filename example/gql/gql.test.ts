  ...
  describe('GraphQL JSON Schema File', () => {
    it('should load and cache GraphQL schema from JSON file', async () => {
      ...
      assert.strictEqual(
        docs[0].name,
        'TestGraphQLJSON',
        'Cache entry name should match'
      );
      assert.ok(docs[0].resources.length > 0, 'Should have resources');

      const queryResources = docs[0].resources.filter(
        (r) => r.type === 'query'
      );
      ...
      assert.strictEqual(
        ...
        'Get user by ID',
        'User query should have correct description'
      );
      ...
      assert.strictEqual(
        ...
        'Get all users',
        'Users query should have correct description'
      );

      const mutationResources = docs[0].resources.filter(
        (r) => r.type === 'mutation'
      );
      assert.ok(mutationResources.length > 0, 'Should have mutation resources');

      const createUserMutation = mutationResources.find(
        (r) => r.name === 'createUser'
      );
      assert.ok(createUserMutation, 'Should have createUser mutation');
      assert.strictEqual(
        createUserMutation.description,
        'Create a new user',
        'CreateUser mutation should have correct description'
      );
    });
    ...
    it('should retrieve specific method details from GraphQL JSON schema', async () => {
      ...
      assert.strictEqual(
        details[0].resources[0].name,
        'user',
        'Should match query name'
      );
