...
export const fetchApiSchema = async (source: UrlSource): Promise<string> => {
...
export const fetchGrpcSchema = async (source: UrlSource): Promise<string> => {
    const response = await fetchSchema(source.url, source.method, source.headers);
    if (!response.ok) {
        throw new Error(`Failed to fetch gRPC schema from ${source.url}: ${response.statusText}`);
    }
